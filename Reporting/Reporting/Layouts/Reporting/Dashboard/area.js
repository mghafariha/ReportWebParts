...
$(document).ready(function () {
    ...
    $('#searchText').on('input', function () {
        createAreasBox($(this).val());
    })


    function createAreasBox(txt) {
        var html = ''
        html += '<ul class="fahter" >';
        var myAreas = areas;
        if (txt)
            myAreas = areas.filter(function (x) { return x.Title.indexOf(txt) != -1 });
        for (var i = 0; i < myAreas.length; i++) {
            var a = areas[i];
            html += '<li type="area" value="' + a.Id + '" class="areas">' + a.Title + '</li><ul class="child">'
    ...
    function areaClick() {
      ...
      //  $('#pag-loadeDisable').attr('id', 'pag-loade');
        ...
        jQuery.when(getAreaReport(areaID)).done(function (a) {
             area = JSON.parse(a.d)[0];
        ...
        //getInvoiceData(null,areaID).done(function(e){
        ...
        //    millis = Date.now() - start;
        //    start = Date.now();
        //    console.log('صورت وضعيت:' + millis);
        //});
        //getEvalData(areaID).done(function (e) {
        //    eva = JSON.parse(e.d);
        //    calcEvalSection();
        //    millis = Date.now() - start;
        //    start = Date.now();
        ...
        getExecMonthlyData(null,areaID).done(function (e) {
           ...
           // ejraRavandChart('net');
            ...
            getTahvilData(null,areaID).done(function (e) {
                tahArray = JSON.parse(e.d);
                tahvil('net');
                tahvilContract('net');
                $('#tahvil1Select').on('change', function () {
                    tahvil(this.value);
                    tahvilContract(this.value);
                });
               // tahvil('net');
        ...
        getExecOperaData(null,areaID).done(function (e) {
        ...
        //getTahvilByYear(null,areaID).done(function (e) {
        //    tahY = JSON.parse(e.d);
    ...
    function createAreaSection() {
        ...
        $('#area_title').text(area.Area);
        $("#count_jari").text(area.Jari);
        $("#count_enteghali").text(area.Enteghali);
        $('#count_contracts').text(area.Jari + area.Enteghali);
        $('#count_contractors').text(area.ContractorN);
        $('#count_consultants').text(area.ConsultantN);
        $('#count_managers').text(area.ManagerN);
        $('#sum_drain').text(area.Drain);
        $('#sum_equipp').text(area.Equipp);
        $('#sum_network').text(area.Network);
    ...
    function tahvil(type) {
        ...
        switch (type) {
            case 'net':
                t_totalSpace =sum( tahArray,'NetworkFinal');
                t_execSpace = execM[execM.length - 1].NetCum;
                t_readySpace = sum(tahArray, 'ReadyNetWork'); 
                t_docSpace =sum(tahArray,'NetworkDoc');
                t_deliverSpace =sum(tahArray,'NetworkDelivered');
                t_deliverRemovedSpace =sum(tahArray,'NetworkRemove');
                t_deliverWithoutSpace =sum( tahArray,'NetworkDelivered3');
                t_deliver2Space =sum( tahArray,'NetworkFinalDeliver');
                break;
            case 'dar':
                t_totalSpace =sum(tahArray,'FinalDrain');
                t_execSpace = execM[execM.length - 1].DrainCum;
                t_readySpace =sum( tahArray,'ReadyDrainage');
                t_docSpace =sum( tahArray,'DraindDoc');
                t_deliverSpace =sum( tahArray,'DrainDelivered');
                t_deliverRemovedSpace =sum( tahArray,'DrainRemove');
                t_deliverWithoutSpace =sum( tahArray,'DrainDelivered3');
                t_deliver2Space = sum(tahArray,'DrainFinalDeliver');
                break;
            case 'equ':
                t_totalSpace =sum( tahArray,'FinalEquipp');
                t_execSpace = execM[execM.length - 1].EquipCum;
                t_readySpace =sum( tahArray,'ReadyEquipped');
                t_docSpace =sum( tahArray,'EquipDoc');
                t_deliverSpace =sum(tahArray,'EquippedDelivered');
                t_deliverRemovedSpace =sum(tahArray,'EquippedRemove');
                t_deliverWithoutSpace =sum(tahArray,'EquippedDelivered3');
                t_deliver2Space =sum( tahArray,'EquippedFinalDeliver');
                break;
        }
    ...
    function tahvilContract(type) {
        var chartData=[];
        for (var i = 0; i < tahArray.length; i++) {
            var obj={};
            var t_totalSpace, t_execSpace, t_readySpace, t_docSpace, t_deliverSpace, t_deliverWithoutSpace, t_deliverRemovedSpace, t_deliver2Space;
            switch (type) {
                case 'net':
                  obj.t_totalSpace = tahArray[i].NetworkFinal + tahArray[i].FinalDrain +tahArray[i].FinalEquipp;
                  obj.t_execSpace =execM[execM.length - 1].NetCum;
                  obj. t_readySpace = tahArray[i].ReadyNetWork;
                  obj. t_docSpace = tahArray[i].NetworkDoc;
                  obj. t_deliverSpace = tahArray[i].NetworkDelivered;
                  obj. t_deliverRemovedSpace = tahArray[i].NetworkRemove;
                  obj. t_deliverWithoutSpace = tahArray[i].NetworkDelivered3;
                  obj. t_deliver2Space = tahArray[i].NetworkFinalDeliver;
                    break;
                case 'dar':
                    obj. t_totalSpace = tahArray[i].NetworkFinal + tahArray[i].FinalDrain + tahArray[i].FinalEquipp;
                    obj. t_execSpace = execM[execM.length - 1].DrainCum;
                    obj. t_readySpace = tahArray[i].ReadyDrainage;
                    obj. t_docSpace = tahArray[i].DraindDoc;
                    obj. t_deliverSpace = tahArray[i].DrainDelivered;
                    obj. t_deliverRemovedSpace = tahArray[i].DrainRemove;
                    obj. t_deliverWithoutSpace = tahArray[i].DrainDelivered3;
                    obj. t_deliver2Space = tahArray[i].DrainFinalDeliver;
                    break;
                case 'equ':
                    obj. t_totalSpace = tahArray[i].NetworkFinal + tahArray[i].FinalDrain + tahArray[i].FinalEquipp;
                    obj.  t_execSpace = execM[execM.length - 1].EquipCum;
                    obj.  t_readySpace = tahArray[i].ReadyEquipped;
                    obj. t_docSpace = tahArray[i].EquipDoc;
                    obj. t_deliverSpace = tahArray[i].EquippedDelivered;
                    obj.t_deliverRemovedSpace = tahArray[i].EquippedRemove;
                    obj. t_deliverWithoutSpace = tahArray[i].EquippedDelivered3;
                    obj.t_deliver2Space = tahArray[i].EquippedFinalDeliver;
                    break;
            }
            chartData.push(obj);
        }
        ...
        var yAxis = { title: 'هكتار' };
              //  stackingColumnOptions(Data, xAxis, seris, yAxis, title, subTitle, legend, toolTip, percentage, type, drill)
        var o = stackingColumnOptions([], {}, [], yAxis, "وضعیت تحویل", "", { enable: true }, { multi: false }, false, 'fixedPlacement');
        //var o = basicColumnOptions([], {}, [], yAxis, "وضعیت تحویل", "", { enable: true }, { multi: false });
        o.xAxis.categories = tahArray.map(function (a) {return a.ContractName });
       // o.xAxis.categories = ['سطح کل', ' اجراشده', ' قابل بهره برداری', 'ارسال مستندات', 'تحویل شده', 'رفع نقص شده', 'تحویل قطعی'];
        o.series = [{ name: 'تحويل', type: 'column', data: chartData.map(function (a) { return a.t_deliverSpace }) }, { name: 'رفع نقص', type: 'column', data: chartData.map(function (a) { return a.t_deliverRemovedSpace + a.t_deliverWithoutSpace }) }];
        //var o2 = activityGaugeOptions(yAxis2, series2, 'وضعیت تحویل');
